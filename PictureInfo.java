...
public class PictureInfo
{
    ...
    throws Throwable
  {
    ...
    if (dot <= 0 || dot > flen - 4 || dot < flen - 5)
      throw new RuntimeException("missing type type suffix for image file name '"+image+"'");
    ...
    if (alreadyRotated = (image.charAt(dot - 1) == 'r'))
    ...
    else if (rotated.length() != 0)
      {
        ...
        if (rotname.exists())
        ...
        else if (autoScale)
          {
            ...
            String[] magickArgs = {"magick", image,
                                   "-rotate", rot,
                                   rotname.toString()};
            Process process = Runtime.getRuntime().exec(useMagick ? magickArgs : jpegtranArgs);
            int exitCode = process.waitFor();
            if (exitCode != 0)
                System.err.println("Unexpected " + (useMagick ? "magick" : "jpegtran") +" exitCode:" + exitCode);
            info.large = ImageInfo.readMetadata(rotname);
            forceReScale = true;
          }
        else
      ...
      {
        info.large = info.original;
        Long orientation = original.getOrientation();
        if (orientation != null)
          {
              long orient = orientation.longValue();
              info.large = new ImageInfo(info.original);
              if (orient >= 5)
                {
                  info.large.width = info.original.height;
                  info.large.height = info.original.width;
                }
          }
      }
    ...
    if (forceReScale || ! path.exists())
      {
        ...
        if (useMagick) {
            String[] magickArgs = {"magick", orig.toString(),
                                   "-auto-orient",
                                   "-scale", maxDim + "x" + maxDim,
                                   filename};
            Process process = Runtime.getRuntime().exec(magickArgs);
            int exitCode = process.waitFor();
            if (exitCode != 0)
                System.err.println("Unexpected magick exitCode:" + exitCode);
        } else {
  ...
  {
    if (large != null && filename.equals(large.filename))
      return large;
    else if (original != null && filename.equals(original.filename))
      return original;
  ...
  public ImageInfo findScaledImage (String scaleName)
  {
    if (scaleName == null || scaleName.length() == 0)
    {
        return original;
        /*
        if (original.filename.toString().indexOf("01") >= 0)
              new Error("findScaledImage "+scaleName+" for "+original.filename).printStackTrace();
      return large;
        */
    }
